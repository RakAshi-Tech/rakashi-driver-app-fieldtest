...
/**
 * How a row is tied to a driver.
 *
 *  own              - ownerColumn must equal the caller's driver id
 *  own_or_unassigned- ownerColumn equals the caller OR is NULL. Used for the job
 *                     board: open jobs have driver_id NULL until someone accepts.
 *  via              - ownership lives on a parent row (gps_track_points -> summary)
 *  unowned          - no owner column exists. Reads are never allowed for these;
 *                     only the writes listed in `operations` get through.
 */
export type OwnershipKind = 'own' | 'own_or_unassigned' | 'via' | 'unowned'

export interface ViaOwnership {
  /** Column on this table holding the parent row id. */
  column: string
  /** Parent table that carries the real owner column. */
  parentTable: string
  /** Primary key column on the parent. */
  parentKey: string
  /** Owner column on the parent. */
  parentOwnerColumn: string
}

export interface TablePolicy {
  ownership: OwnershipKind
  /** Column holding the driver id. Required for 'own' and 'own_or_unassigned'. */
  ownerColumn?: string
  via?: ViaOwnership
  operations: Operation[]
  /**
   * Columns the client may supply on insert/upsert/update. Owner columns are
   * deliberately absent - the handler injects those from the verified token.
   */
  writable: string[]
  /** Conflict targets permitted for upsert. Anything else is rejected. */
  upsertOn?: string[]
}
...
export function policyFor(table: string): TablePolicy | null {
  return Object.prototype.hasOwnProperty.call(POLICIES, table)
    ? POLICIES[table]
    : null
}
